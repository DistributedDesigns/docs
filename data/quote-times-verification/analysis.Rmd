...
quotes <- read.csv("quotes.csv", header = T, col.names = c("time", "attempts"))
quotes$time <- quotes$time / 1e6 # convert ns -> ms
```
What's the response time distribution for single attempt quotes? (times in ms)
```{r single attempt times, echo=FALSE}
singleAttempts <- subset(quotes, attempts==1)
...
plot(h,
     ...
     main = bquote("Single attempt response time," ~ n == 2727)
     )
```
...
```{r hist, echo=FALSE}
htime = hist(quotes$time, plot=F)
htime$density = htime$density * 1e1 # change density to percents
plot(htime,
     freq = F,
     col  = "lightgreen",
     ylim = range(0,0.2),
...
hattempts = hist(quotes$attempts, breaks = c(seq(1, 8)), plot=F)
plot(hattempts,
     freq = F,
     col  = "lightgreen",
     ylim = range(0,1),
